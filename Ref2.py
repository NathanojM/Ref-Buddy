...
def newproj(event=None):
    global newprojbox
    newprojbox=Entry(filegroup,bg="lightgreen")
    newprojbox.grid(row=0,column=0)
    ribbon.select(filetab)
    newprojbox.focus_set()
    newprojbox.bind("<Return>",makenewproj)
    newprojbox.bind("<Escape>",nonewproj)

   
def makenewproj(event=None):
    if int(len(newprojbox.get()))>0:
